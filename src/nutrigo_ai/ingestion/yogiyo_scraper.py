# yogiyo_store_menu_scraper_final.py
# Python 3.9+ / pip install -U playwright  후  playwright install
# 실행: python yogiyo_store_menu_scraper_final.py

from playwright.sync_api import sync_playwright, TimeoutError
import json, time, re, os, sys

# ======= 설정 =======
ADDRESS_TEXT = "서울특별시 동작구 흑석로 84"      # 테스트용 주소
STORE_ID = "229998"
STORE_URL = f"https://www.yogiyo.co.kr/mobile/#/{STORE_ID}/"

# geolocation은 frontyo 파라미터로도 재사용함
LAT, LNG = 37.5080617, 126.95999855
ORDER_SERVING_TYPE = "delivery"  # 배달: delivery / 포장: takeaway 등
...
def normalize_frontyo_aggregation(payload: dict):
    """
    ...
    # 옵션 상세 맵 후보(없어도 동작)
    option_item_maps = []
    for k in ["option_items", "optionItemMap", "optionItems", "option_menu_items", "optionItemsMap"]:
    ...
    def _price_from_price_obj(pobj):
        if not isinstance(pobj, dict):
            return None
        for key in ["final_price", "origin_price", "final_price_with_required_options", "origin_price_with_required_options"]:
        ...
        for m in option_item_maps:
            if isinstance(m, dict) and oid in m:
                ...
                try:
                    ...
                    price = 0
                soldout = bool(node.get("soldout") or node.get("sold_out") or node.get("isSoldOut"))
                return {"id": oid, "name": name, "price": price, "soldout": soldout}
...
def normalize_menu(payload):
    ...
    for sec in sections:
        ...
        for it in items:
            ...
            try:
                price = int(str(price_raw).replace(",", "")) if price_raw not in (None, "") else None
            except Exception:
                price = None
            image = it.get("original_image") or it.get("image") or ""
            norm_items.append({
                "name": name, "desc": desc, "price": price, "image": image, "options": []
            })
    return {"items": norm_items, "schema": "generic_items_v1"}

# ======= 메인 작업 =======
def fetch_store_info(store_id: str):
    with sync_playwright() as p:
        context = p.chromium.launch_persistent_context(
            user_data_dir="./yogiyo_profile",
            headless=False,
            slow_mo=100,
            locale="ko-KR",
        ...
        def json_file_count():
            return len([f for f in os.listdir("api_dump") if f.lower().endswith(".json")])

        def on_response(resp):
            try:
                url = resp.url
                status = resp.status
                text = resp.text()  # 비JSON이면 예외 날 수도 있음 → except에서 무시
            except Exception:
                return
            data = None
            ...
            if data is not None:
                ...
                idx = json_file_count()
                with open(os.path.join("api_dump", f"{idx:03d}.json"), "w", encoding="utf-8") as f:
                    json.dump({"url": url, "status": status, "data": data}, f, ensure_ascii=False, indent=2)
                # 디버깅
        ...
        def pick_menu_json():
            ...
            for url, status, data, size in captured:
                score = 0
                if MENU_URL_RE.search(url or ""): score += 4
                score += min(size // 2000, 6)
                if isinstance(data, dict) and ("menu_sections" in data and "menu" in data): score += 5
                elif isinstance(data, dict) and ("items" in data): score += 1
        ...
        # UI 헬퍼
        def wait_any_ui(*locators, timeout=8000):
            for loc in locators:
                try:
        ...
        try:
            # 1) 홈 → 주소 확정
            page.goto("https://www.yogiyo.co.kr/mobile/#/", wait_until="domcontentloaded")
            try:
                addr_box = page.locator("input[placeholder*='건물명'], input[placeholder*='도로명'], input[placeholder*='지번']").first
                addr_box.wait_for(timeout=10000)
                addr_box.click()
                addr_box.fill(ADDRESS_TEXT)

                picked = False
                for sel in ["ul[class*='suggest'] li", "div[class*='suggest'] li", "li:has-text('동작구')", "li"]:
                    try:
                ...
                if not picked:
                    ...
                    try:
                        page.get_by_role("button", name="검색").first.click(timeout=2000)
                    except:
                        pass

                page.wait_for_function("""
                    () => {
                        const el = document.querySelector("input[placeholder*='건물명'],input[placeholder*='도로명'],input[placeholder*='지번']");
                        return el && el.value && el.value.length >= 4;
                    }""", timeout=8000)
                time.sleep(1.0)

                # 간헐 팝업 닫기
                for b in [page.get_by_role("button", name="확인"),
                          page.get_by_role("button", name="닫기"),
                          page.get_by_role("button", name="동의")]:
                    try: b.first.click(timeout=800)
                    except: pass
            except TimeoutError:
            ...
            if not wait_any_ui(page.locator("ul.nav-tabs"),
            ...
            try:
                ...
                try:
                    page.locator("ul.nav-tabs").get_by_text("메뉴").first.click(timeout=3000)
                except TimeoutError:
                    print("[알림] '메뉴' 탭 클릭 실패(이미 메뉴 탭일 수 있음)", file=sys.stderr)
            ...
            # 5) 실패 시 직접 요청(fallback) – 네가 본 정확한 엔드포인트
            if not menu_pick:
                agg_url = (
                ...
                r = context.request.get(agg_url, headers={"Accept": "application/json"})
                if r.ok:
                    data = r.json()
                    menu_pick = {"url": agg_url, "data": data}

            if not menu_pick:
                raise TimeoutError("메뉴 JSON 확보 실패(네트워크 캡처/직접요청 모두 실패)")
            ...
            def text_of(*locs):
                ...
                return ""
            name = text_of(page.locator(".restaurant-name"),
                           page.locator("h2"), page.locator("h3"), page.locator(".name"))
            address = text_of(page.locator("div.info-item:has-text('주소') .info-text"))
            ...
            if "sections" in menu_norm:
                total_items = sum(len(s.get("items", [])) for s in menu_norm["sections"])
            ...
            result = {
                ...
                "menu_json": menu_norm  # 프런트/DB가 표준 스키마만 필요하면 이거 쓰면 됨
            }
